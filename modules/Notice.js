...
// 改为在 init() 后通过函数延迟求值
const NOTICE_DIR  = "./plugins/NEssential/data/NoticeSettingsData/";
...
function _registerNoticeCmd() {
    ...
    cmd.setCallback((_cmd, ori, _out, _res) => {
        ...
        const rawContent = (() => { try { return fs.readFileSync(NOTICE_FILE,'utf8'); } catch(e){ return null; } })() || "暂无公告";
...
function _registerNoticeSetCmd() {
    ...
    noticeSetCmd.setCallback((_cmd, ori, output) => {
        ...
        const currentNotice = fs.existsSync(NOTICE_FILE)
            ? ((() => { try { return fs.readFileSync(NOTICE_FILE,'utf8'); } catch(e){ return null; } })() || "")
            : "";

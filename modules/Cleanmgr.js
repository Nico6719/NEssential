...
/**
 * NEssential - CleanMgr Module (LSE-safe Ultimate)
 * 修改版 v4：
...
var CleanMgr = (function () {
  ...
  // 玩家数据路径: plugins/NEssential/data/CleanmgrSettingData.json
  var DATA_DIR = BASE + "data/";
  var PLAYER_SETTINGS_PATH = DATA_DIR + "CleanmgrSettingData.json"; 

  /* ================= 默认语言 ================= */
  var DEFAULT_LANG = {
    prefix: "§l§6[-NEST-] §l§e[清理系统] §r",
    toast_title: "§l§6[-NEST-] §l§e[清理系统] §r",
    messages: {
  ...
  /* ================= 工具 ================= */
  function ensureDir(p) { if (!fs.existsSync(p)) File.mkdir(p); }
  ...
  function loadLang() {
    ...
    if (!fs.existsSync(LANG_PATH)) {
      fs.writeFileSync(LANG_PATH, JSON.stringify(DEFAULT_LANG, null, 2), 'utf8');
    }
    try { 
      var loaded = JSON.parse((() => { try { return fs.readFileSync(LANG_PATH,'utf8'); } catch(e){ return null; } })());
  ...
  function loadConfig() {
    ...
    var raw = {};
    try { raw = JSON.parse((() => { try { return fs.readFileSync(CONFIG_PATH,'utf8'); } catch(e){ return null; } })()); } catch (e) {
  ...
  function loadPlayerSettings() {
    ensureDir(DATA_DIR); 
    if (fs.existsSync(PLAYER_SETTINGS_PATH)) {
      try {
        playerSettings = JSON.parse((() => { try { return fs.readFileSync(PLAYER_SETTINGS_PATH,'utf8'); } catch(e){ return null; } })());
      } catch (e) {
        playerSettings = {};
  ...
  function savePlayerSettings() {
    ensureDir(DATA_DIR);
    fs.writeFileSync(PLAYER_SETTINGS_PATH, JSON.stringify(playerSettings, null, 2), 'utf8');
  }
